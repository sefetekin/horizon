...
writer = SummaryWriter("sheet2/transferlearningsave")
...
#test_dataloader = torch.utils.data.DataLoader(trainset, batch_size=15, shuffle=True)
...
cnn_neural_network.fc = torch.nn.Linear(in_features=1024, out_features=2, bias=True)
...
count = 0
for epoch in range(1):  # loop over the dataset multiple times to make it more precia=
...
torch.save(cnn_neural_network.state_dict(), 'models_transfer/model1.pk')
