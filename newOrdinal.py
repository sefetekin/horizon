...
def ordinal_crossentropy(predictions, targets):
    ...
    cost_calc = torch.nn.CrossEntropyLoss(reduction='none')(predictions, modified_target).sum(axis=1).sum()
    return(cost_calc)
...
writer = SummaryWriter("sheet2/newOrdinalCost3")
...
cnn_neural_network.fc = torch.nn.Linear(in_features=512, out_features=3, bias=True)
