...
def ordinal_crossentropy(predictions, targets):
    ...
    cost_calc = torch.nn.MSELoss(reduction='none')(predictions, modified_target).sum(axis=1).sum()
    return(cost_calc)
...
writer = SummaryWriter("sheet2/OrdinalCost")


weights = []
for path, label in trainset.imgs:
    if label == 0:
        weights.append(2286/2286)
    elif label == 1:    
        weights.append(2286/1046)
    elif label == 2:
        weights.append(2286/1516)
    elif label == 3:
        weights.append(2286/930)
...
for path, label in testset.imgs:
    if label == 0:
        weights_test.append(639/639)
    elif label == 1:    
        weights_test.append(639/296)
    elif label == 2:
        weights_test.append(639/447)
    elif label == 3:
        weights_test.append(639/274)
...
cnn_neural_network.fc = torch.nn.Linear(in_features=512, out_features=4, bias=True)
#infuture is 
...
count=0
#30-45 minutes
for epoch in range(10):  # loop over the dataset multiple times to make it more precia=
    for training_data in train_dataloader: #traverse over the batches
        ...
        if count % 5 == 0:
            ...
            cost_test = ordinal_crossentropy(model_predictions_test, test_data_labels)
        ...
        precision, recall, f1, support = precision_recall_fscore_support(training_data_labels,
                                                ...
                                                zero_division=0,
                                                labels=(0,1,2,3))
        cost = ordinal_crossentropy(model_predictions, training_data_labels)
